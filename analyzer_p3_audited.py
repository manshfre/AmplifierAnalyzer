...
class AnalysisRule:
    ...
    name: str
    func_name: str
    requires: Tuple[Union[str, Tuple[str, type]], ...] = ()  # 需要存在的属性路径（如 'circuit.current_paths'）
...
class CircuitAnalyzer:
    ...
    class AnalysisConfig:
        ...
        def list_keys(self, prefix: str = "") -> dict[str, str]:
            ...
            for name, value in self.__class__.__dict__.items():
                ...
                # 从实例 map 中获取描述
                description = self._chinese_map.get(name, value)
                result[name] = f"{value} ({description})"
            
            # 2. 遍历动态添加的属性 (Instance Attributes)
            for name in self._dynamic_keys:
                if prefix and not name.startswith(prefix):
                    continue
        ...
        def remove_key(self, attr_name: str):
            """
            [动态扩展] 删除常量 (仅删除动态添加的，静态常量不可删)。
            """
            if not hasattr(self, attr_name):
                logger.error(f"[AnalysisConfig] 错误: 常量 {attr_name} 不存在")
                return
            
            # 检查是否为静态常量
            if hasattr(self.__class__, attr_name):
                logger.warning(f"[AnalysisConfig] 警告: 无法移除静态定义的常量 {attr_name}，操作被忽略。")
                return
            ...
            if attr_name in self._dynamic_keys:
                ...
                self._dynamic_keys.remove(attr_name)
    ...
    class CircuitPorts:
        ...
        # ==================== 审计字段（全局可变配置） ====================
        # __audit_epoch 用于检测进程内的全局配置漂移；任何会修改端口集合的操作都应 bump。
        __audit_epoch: int = 0

        @classmethod
        def audit_epoch(cls) -> int:
            """返回当前审计 epoch（单调递增）。"""
            try:
                return int(cls.__audit_epoch)
            except Exception:
                return int(getattr(cls, "_CircuitPorts__audit_epoch", 0))

        @classmethod
        def _bump_audit_epoch(cls, action: str, **details: Any) -> None:
            """修改端口配置后调用；用于审计与测试隔离。"""
            try:
                cls.__audit_epoch = cls.audit_epoch() + 1
                payload: Dict[str, Any] = {"epoch": cls.__audit_epoch, "action": action}
                if details:
                    payload["details"] = details
                try:
                    payload_s = CircuitAnalyzer._sanitize_for_json(payload)
                except Exception:
                    payload_s = payload
                logger.info(f"[CircuitPorts] audit_epoch_bump: {payload_s}")
            except Exception:
                pass
        ...
        def add_port(cls, name: str, value_set: Set[str]) -> None:
            ...
            logger.info(f"[CircuitPort] 已添加端口常量: {name} = {value_set}")
            cls._bump_audit_epoch("add_port", name=name)
        ...
        def remove_port(cls, name: str) -> None:
            ...
            if hasattr(cls, name):
                delattr(cls, name)
                logger.info(f"[CircuitPort] 已删除端口常量: {name}")
                cls._bump_audit_epoch("remove_port", name=name)
            else:
        ...
        def add_to_port(cls, name: str, element: str) -> None:
            ...
            current.add(element)
            logger.info(f"[CircuitPort] 已添加 '{element}' 到 {name}")
            cls._bump_audit_epoch("add_to_port", name=name, element=element)
        ...
        def remove_from_port(cls, name: str, element: str) -> None:
            ...
            if element in current:
                ...
                logger.info(f"[CircuitPort] 已从 {name} 删除 '{element}'")
                cls._bump_audit_epoch("remove_from_port", name=name, element=element)
            else:
                logger.warning(f"[CircuitPort] 警告: '{element}' 不存在于 {name} 中")
        ...
        def reset_to_defaults(cls) -> None:
            ...
            logger.info("[CircuitPort] 已重置所有端口为默认值")
            cls._bump_audit_epoch("reset_to_defaults")
    ...
    class APPConfig:
        ...
        **键（大写标识符）代表逻辑上的各种器件类型，不可增删改，仅可修改其对应值**
        ...
        # 值: 分析器中的别名 (Alias)
        # ==================== 审计字段（全局可变配置） ====================
        # __audit_epoch 用于检测进程内的全局配置漂移；任何会修改配置的操作都应 bump。
        __audit_epoch: int = 0
        ...
        def audit_epoch(cls) -> int:
            ...
            try:
                ...
                return int(getattr(cls, "_APPConfig__audit_epoch", 0))
        ...
        def _bump_audit_epoch(cls, action: str, **details: Any) -> None:
            ...
            try:
                ...
                try:
                    ...
                    payload_s = payload
                logger.info(f"[APPConfig] audit_epoch_bump: {payload_s}")
            except Exception:
                pass
        ...
        def add_device_type(cls, key: str, default_alias: str) -> None:
            ...
            logger.info(f"[APPConfig] 新增器件类型: {key} (别名: {default_alias})")
            cls._bump_audit_epoch("add_device_type", key=key)
            logger.warning(f"  RefCheck -> 请检查是否已设计该器件别名相关处理逻辑？")
            logger.warning(f"  RefCheck -> 请检查是否已配置 PORT_CONFIG[{key}] 并设计了相关端口处理逻辑？")
            logger.warning(f"  RefCheck -> 请检查是否已配置 PARAM_CONFIG[{key}] 并设计了相关参数处理逻辑？")
        ...
        def add_port_config(cls, key: str, ports: Set[str]) -> None:
            ...
            logger.info(f"[APPConfig] 新增端口定义: {key} -> {ports}")
            cls._bump_audit_epoch("add_port_config", key=key)
            logger.warning(f"  RefCheck -> 请确认该端口集合是否完整描述了器件物理特性？")
            logger.warning(f"  RefCheck -> 请检查是否有相关逻辑处理这些端口（如栅极识别等）？")
            logger.warning(f"  RefCheck -> 请检查是否已配置 ALIAS_CONFIG[{key}] 并设计了相关器件处理逻辑？")
            logger.warning(f"  RefCheck -> 请检查是否已配置 PARAM_CONFIG[{key}] 并设计了相关参数处理逻辑？")
        ...
        def remove_entry_strictly(cls, key: str) -> None:
            ...
            if key not in cls.ALIAS_CONFIG and key not in cls.PORT_CONFIG:
                logger.warning(f"[APPConfig] 警告: 尝试删除不存在的键 '{key}'")
                return

            logger.warning(f"[APPConfig] 警告: 正在删除核心定义 '{key}'！")
            logger.info(f"  RiskCheck -> 请检查现有分析器逻辑是否与被删除器件类型完全无关？")
            logger.info(f"  RiskCheck -> 请检查另外两字典条目是否也应一并删除？")
            
            if key in cls.ALIAS_CONFIG:
                del cls.ALIAS_CONFIG[key]
                logger.info(f"  - 已删除 ALIAS_CONFIG[{key}]")
            
            if key in cls.PORT_CONFIG:
                del cls.PORT_CONFIG[key]
                logger.info(f"  - 已删除 PORT_CONFIG[{key}]")

            cls._bump_audit_epoch("remove_entry_strictly", key=key)
        ...
        def update_param_config(cls, key: str, params: Set[str]) -> None:
            ...
            if not is_new:
                logger.info(f"[APPConfig] 修改已有参数配置: {key}")
                logger.info(f"  LogicCheck -> 请检查现有参数校准/约束逻辑是否适配修改后的参数列表？")
            else:
                logger.info(f"[APPConfig] 新增参数配置: {key}")
                logger.warning(f"  RefCheck -> 请确保 ALIAS_CONFIG[{key}] 和 PORT_CONFIG[{key}] 已存在。")
                logger.info(f"  LogicCheck -> 请确保已实现对应的参数提取与校准逻辑。")

            cls.PARAM_CONFIG[key] = params.copy()
            cls._bump_audit_epoch("update_param_config", key=key)
            logger.info(f"  - 当前 {key} 参数集合: {params}")
        ...
        def reset_to_defaults(cls) -> None:
            ...
            logger.info("[APPConfig] 已重置所有配置为默认值")
            cls._bump_audit_epoch("reset_to_defaults")
    ...
    # ------------------------------
    @staticmethod
    def _sanitize_for_json(obj: Any, _depth: int = 0, _seen: Optional[Set[int]] = None) -> Any:
        """将对象递归转换为 JSON-safe 且确定性的结构。

        设计目标：
        - 作为审计/回归输出的底座：相同输入应产生确定性 context/stats；
        - 避免 set/dict 遍历顺序导致日志与报告不稳定；
        - 避免不可序列化对象（type/Enum/异常/自定义对象等）导致运行时错误。
        """
        if _seen is None:
            _seen = set()

        # 当前调用相对于初始调用（深度 0）已经深入了多少层嵌套结构（dict/list/tuple/set/对象展开后的属性）
        # (1,(1,(2,3)))深度是3
        if _depth > 10:
            return "<MaxDepth>"
        ...
        # 映射类型：键强制为 str 并排序以确定性输出
        if isinstance(obj, dict):
            out: Dict[str, Any] = {}
            for k in sorted(obj.keys(), key=lambda x: str(x)):
                out[str(k)] = CircuitAnalyzer._sanitize_for_json(obj[k], _depth=_depth + 1, _seen=_seen)
            return out

        # 序列类型：保持顺序
        if isinstance(obj, (list, tuple)):
            return [CircuitAnalyzer._sanitize_for_json(v, _depth=_depth + 1, _seen=_seen) for v in obj]

        # 集合：排序以确定性输出
        if isinstance(obj, set):
            items = [CircuitAnalyzer._sanitize_for_json(v, _depth=_depth + 1, _seen=_seen) for v in obj]
            return sorted(items, key=lambda x: str(x))

        # 其他对象：尽量展开 __dict__，否则降级为 str
        try:
            if hasattr(obj, "__dict__"):
                return CircuitAnalyzer._sanitize_for_json(vars(obj), _depth=_depth + 1, _seen=_seen)
        except Exception:
            pass
    ...
    def _emit(self, level: str, message: str, exc_info: Optional[Union[bool, BaseException]] = None, **context: Any) -> None:
        """统一事件出口：logger + report。

        修复点：
        - logger 侧必须能输出 traceback：在 except 语境中传 exc_info=True。
        - logger 侧不能丢弃上下文：在 error/critical 至少拼接关键字段到 message。
        - report 写入失败不应完全静默：debug 级别留痕（不影响主流程）。
        """
        level_lower = (level or "info").lower()

        # 仅抽取关键上下文字段拼接到 message，避免日志过长与泄露大对象
        if level_lower in ("error", "critical"):
            keys = ("rule", "func_name", "error", "missing", "requires", "device", "net", "struct_key")
        elif level_lower == "warning":
            keys = ("rule", "func_name", "missing", "device", "net", "struct_key")
        else:
            keys = ("rule", "func_name")

        safe_context = self._sanitize_for_json(context)
        ctx_for_msg: Dict[str, Any] = {}
        for k in keys:
            if isinstance(safe_context, dict) and k in safe_context:
                ctx_for_msg[k] = safe_context[k]

        suffix = ""
        if ctx_for_msg:
            try:
                suffix = " | " + json.dumps(ctx_for_msg, ensure_ascii=False, sort_keys=True)
            except Exception:
                suffix = " | " + str(ctx_for_msg)

        log_message = f"{message}{suffix}"
        ...
        }.get(level_lower, self.logger.info)

        # exc_info=True 会打印当前异常堆栈（应在 except 块内调用）
        want_exc = bool(exc_info is True or isinstance(exc_info, BaseException))

        # 若 formatter 支持，可从 extra.context 读取完整上下文（例如 JSON handler）
        try:
            if want_exc:
                log_fn(log_message, exc_info=True, extra={"context": safe_context})
            else:
                log_fn(log_message, extra={"context": safe_context})
        except TypeError:
            # 某些 handler/formatter 不接受 extra，降级输出
            if want_exc:
                log_fn(log_message, exc_info=True)
            else:
                log_fn(log_message)

        # report 收敛：保留完整 context（包含 traceback 字符串）
        try:
            if isinstance(safe_context, dict):
                self.report.add_event(level_lower, message, **safe_context)
            else:
                self.report.add_event(level_lower, message, context=safe_context)
        except Exception:
            # report 不应影响主流程，但也不应完全静默
            try:
                self.logger.debug("report.add_event 写入失败", exc_info=True)
            except Exception:
                pass
    ...
    def export_report(self) -> Dict[str, Any]:
        ...
        # [确定性] 审计输出必须是 JSON-safe 且稳定的
        return self._sanitize_for_json(raw)


    # ------------------------------
    # [Audit] 变量/状态审计与可重入性支持
    # ------------------------------
    # 审计分层：仅用于报告与校验，不改变业务语义
    _AUDIT_STATE_LAYERS: Dict[str, Tuple[str, ...]] = {
        "sot": (
            "circuit.devices_dict",
            "net_device_map",
            "device_params",
            "config_complete",
        ),
        ...
        "derived": (
            "circuit.substructures",
            "circuit.current_paths",
            "circuit.current_beams",
            "circuit.current_beam_paths",
            "circuit.beam_net_sets",
            "constraint_groups",
            "copy_tube_r_raw_map",
            "calibrate_params",
            "top_nodes",
    ...
    # Cache -> 依赖源（用于失效与一致性校验）
    _AUDIT_CACHE_DEPENDENCIES: Dict[str, Tuple[str, ...]] = {
        "_flat_beam_cache": ("circuit.current_beam_paths",),
    }

    @staticmethod
    # 查找环境变量的设置，没有返回自行设置标志default
    ...
    def _variable_audit_spec(self) -> Dict[str, Any]:
        """返回变量审计规范（用于报告固化与回归解释）。"""
        spec = {
            "state_layers": self._AUDIT_STATE_LAYERS,
            "cache_dependencies": self._AUDIT_CACHE_DEPENDENCIES,
            "validate_state_env": "ANALYZER_VALIDATE_STATE / ANALYZER_VALIDATE_STATE_STRICT",
            "snapshot_env": "ANALYZER_RECORD_STATE_SNAPSHOTS / ANALYZER_STATE_SNAPSHOT_DIGEST",
            "rule_snapshots_env": "ANALYZER_RECORD_RULE_SNAPSHOTS",
        }
        return self._sanitize_for_json(spec)

    def _invalidate_beam_cache(self, reason: str) -> None:
        """失效电流束派生缓存。"""
        if self._flat_beam_cache is not None:
            try:
                inv = self.report.stats.get("cache_invalidations")
                if not isinstance(inv, dict):
                    inv = {}
                inv[reason] = int(inv.get(reason, 0)) + 1
                self.report.stats["cache_invalidations"] = inv
            except Exception:
                pass
    ...
    def _reset_report_for_run(self) -> None:
        ...
        if self._baseline_report is None:
            try:
                ...
                self._baseline_report = {"events": [], "calibrations": [], "stats": {}}

        base = self._baseline_report or {"events": [], "calibrations": [], "stats": {}}
        try:
            ...
            self.report.events = []
            self.report.calibrations = []
            self.report.stats = {}

    def _reset_derived_state_for_run(self) -> None:
        """清理派生态（Derived + Graph-Driven Storage + Cache），保留 SoT。"""
        # Graph-Driven Storage
        try:
        ...
        try:
            ...
            self.relation_graph = defaultdict(lambda: defaultdict(list))
        try:
        ...
        try:
            ...
            self.circuit.substructures = []
            self.circuit.current_paths = []
            self.circuit.current_beams = {}
            self.circuit.current_beam_paths = {}
            self.circuit.beam_net_sets = {}

        # Device-level derived (tags/substructures)
        try:
        ...
        # Cache
        self._invalidate_beam_cache("reset_derived_state")

    def _get_global_config_snapshot(self) -> Dict[str, Any]:
        """抓取进程内全局可变配置快照（JSON-safe 前的原始结构）。"""
        return {
            "CircuitPorts": self.CircuitPorts.list_ports(),
            "CircuitPorts_epoch": self.CircuitPorts.audit_epoch(),
            "APPConfig": self.APPConfig.get_config_summary(),
            "APPConfig_epoch": self.APPConfig.audit_epoch(),
        }

    def _audit_global_config(self, stage: str) -> None:
        """记录全局可变配置审计信息，并检测漂移。"""
        try:
            snap_raw = self._get_global_config_snapshot()
            snap = self._sanitize_for_json(snap_raw)
            payload_json = json.dumps(snap, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
            digest = hashlib.sha256(payload_json.encode("utf-8")).hexdigest()
            rec = {
                "epoch_ports": snap.get("CircuitPorts_epoch"),
                "epoch_app": snap.get("APPConfig_epoch"),
                "digest": digest,
            }
            g = self.report.stats.get("global_config_audit")
            if not isinstance(g, dict):
                g = {}
            g[stage] = rec
            self.report.stats["global_config_audit"] = g

            if stage == "pipeline_end":
                start = g.get("pipeline_start", {}).get("digest")
                end = g.get("pipeline_end", {}).get("digest")
                if start and end and start != end:
                    self._warn(
                        "检测到进程内全局配置漂移（可能影响可复现性/测试隔离）",
                        start_digest=start,
                        end_digest=end,
                    )
        except Exception as e:
            self._warn("全局配置审计失败", error=str(e))

    def _record_state_snapshot(self, stage: str) -> None:
        """记录轻量状态快照（默认关闭）。"""
        if not self._env_flag("ANALYZER_RECORD_STATE_SNAPSHOTS", default=False):
            return
        try:
            snap: Dict[str, Any] = {
                "stage": stage,
                "devices": len(self.circuit.devices_dict),
                "nets": len(self.net_device_map),
                "tags": {k: len(v) for k, v in self.tag_index.items()},
                "groups": {k: len(v) for k, v in self.device_groups.items()},
                "relations": len(self.relation_graph),
                "substructures": len(self.circuit.substructures),
                "current_paths": len(self.circuit.current_paths),
                "current_beams": len(self.circuit.current_beams),
                "current_beam_paths": len(self.circuit.current_beam_paths),
            }
            if self._env_flag("ANALYZER_STATE_SNAPSHOT_DIGEST", default=False):
                snap["digest"] = self._compute_state_digest()
            arr = self.report.stats.get("state_snapshots")
            if not isinstance(arr, list):
                arr = []
            arr.append(self._sanitize_for_json(snap))
            self.report.stats["state_snapshots"] = arr
        except Exception as e:
            self._warn("记录状态快照失败", stage=stage, error=str(e))
    ...
    def _compute_state_digest(self) -> str:
        ...
        core = {
            ...
            "substructures": [s.sub_id for s in self.circuit.substructures],
            "constraint_groups": self.constraint_groups,
        }
        safe = self._sanitize_for_json(core)
        payload = json.dumps(safe, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        return hashlib.sha256(payload.encode("utf-8")).hexdigest()

    def _validate_state(self, stage: str) -> None:
        """运行期变量一致性校验（默认关闭）。"""
        if not self._env_flag("ANALYZER_VALIDATE_STATE", default=False):
            return
        strict = self._env_flag("ANALYZER_VALIDATE_STATE_STRICT", default=False)
        errors: List[Dict[str, Any]] = []

        # 1) tag_index 引用有效性 + device.tags 双向一致性
        try:
            for tag, names in self.tag_index.items():
                for name in list(names):
                    if name not in self.circuit.devices_dict:
                        errors.append({"type": "dangling_tag_ref", "tag": tag, "name": name})
                    else:
                        dev = self.circuit.devices_dict[name]
                        if tag not in dev.tags:
                            errors.append({"type": "tag_index_device_mismatch", "tag": tag, "name": name})
            for name, dev in self.circuit.devices_dict.items():
                for tag in list(dev.tags):
                    if name not in self.tag_index.get(tag, set()):
                        errors.append({"type": "device_tag_index_mismatch", "tag": tag, "name": name})
        except Exception as e:
            errors.append({"type": "tag_validation_exception", "error": str(e)})

        # 2) device_groups 引用有效性
        try:
            for gk, groups in self.device_groups.items():
                for grp in groups:
                    for name in grp:
                        if name not in self.circuit.devices_dict:
                            errors.append({"type": "dangling_group_ref", "group": gk, "name": name})
        except Exception as e:
            errors.append({"type": "group_validation_exception", "error": str(e)})

        # 3) relation_graph 引用有效性
        try:
            for src, rels in self.relation_graph.items():
                if src not in self.circuit.devices_dict:
                    errors.append({"type": "dangling_relation_src", "src": src})
                for rel, dsts in rels.items():
                    for dst in dsts:
                        if dst not in self.circuit.devices_dict:
                            errors.append({"type": "dangling_relation_dst", "src": src, "rel": rel, "dst": dst})
        except Exception as e:
            errors.append({"type": "relation_validation_exception", "error": str(e)})

        # 4) net_device_map key lower 约束 + 引用有效性
        try:
            for net, names in self.net_device_map.items():
                if str(net) != str(net).lower():
                    errors.append({"type": "net_not_lower", "net": net})
                for name in names:
                    if name not in self.circuit.devices_dict:
                        errors.append({"type": "dangling_net_ref", "net": net, "name": name})
        except Exception as e:
            errors.append({"type": "net_validation_exception", "error": str(e)})

        # 5) cache 一致性（仅在 cache 已建立时校验）
        try:
            if self._flat_beam_cache is not None:
                recomputed: List[Set[str]] = []
                for path in self.circuit.current_beam_paths.values():
                    for s in path:
                        if isinstance(s, set):
                            recomputed.append(set(s))
                a = {frozenset(x) for x in self._flat_beam_cache}
                b = {frozenset(x) for x in recomputed}
                if a != b:
                    errors.append({"type": "beam_cache_mismatch", "cache_size": len(a), "recomputed_size": len(b)})
        except Exception as e:
            errors.append({"type": "cache_validation_exception", "error": str(e)})

        if errors:
            self._warn("变量一致性校验发现问题", stage=stage, issues=errors)
            if strict:
                raise AssertionError(f"State validation failed at {stage}: {errors}")

    def _prepare_for_pipeline_run(self) -> None:
        """pipeline 运行前准备：清理派生态、写入审计规范、记录配置快照。"""
        self._reset_report_for_run()
        self._reset_derived_state_for_run()
        try:
            self.report.stats["variable_audit_spec"] = self._variable_audit_spec()
        except Exception:
            pass
        self._audit_global_config("pipeline_start")
        self._record_state_snapshot("pipeline_start")
        self._validate_state("pipeline_start")

    def _finalize_pipeline_run(self) -> None:
        """pipeline 运行后收尾：审计、快照、校验。"""
        self._record_state_snapshot("pipeline_end")
        self._validate_state("pipeline_end")
        self._audit_global_config("pipeline_end")
    ...
    class DefaultSpiceParser:
        ...
        def check_config_complete(self) -> bool:
            ...
            for name, dev_type, terminals, params in self.devices_info:
                identifier = CircuitAnalyzer.APPConfig.get_identifier_by_type(dev_type)
                required_params = CircuitAnalyzer.APPConfig.get_legal_params(identifier)
                ...
                required_ports = CircuitAnalyzer.APPConfig.get_legal_ports(identifier)
        ...
        def _build_mos(self, name: str, nets: List[str], model: str, rest: str) -> Optional[List[Any]]:
            ...
            dev_type = CircuitAnalyzer.APPConfig.get_device_alias(mos_identifier)

            terminals = {"D": nets[0], "G": nets[1], "S": nets[2], "B": nets[3]}
            params = {"l": l_s, "fw": fw_str, "m": str(nf_int)}

            # 若缺少分析器所需参数（理论上不会到这里），则忽略
            required = CircuitAnalyzer.APPConfig.get_legal_params(mos_identifier)
        ...
        def _build_resistor(self, name: str, nets: List[str], rest: str) -> Optional[List[Any]]:
            ...
            identifier = "RESISTOR"
            dev_type = CircuitAnalyzer.APPConfig.get_device_alias(identifier)
            terminals = {"PLUS": nets[0], "MINUS": nets[1]}
            params = {"segL": l_s, "segW": w_s}

            required = CircuitAnalyzer.APPConfig.get_legal_params(identifier)
            if not required.issubset(params.keys()):
                return None
    ...
    def _build_default_rules(self) -> List[AnalysisRule]:
        ...
        return [
            ...
            # ========================================================
            AnalysisRule(
                name="mark_obvious_tags",
                func_name="_mark_obvious_tags",
                # 需要遍历器件，且需要识别电源/地(隐含依赖Ports配置)
                requires=(
                    ("circuit.devices_dict", dict),
                ),
                description="[Tags] 标记二极管、顶层节点、差分输入、外侧共模管",
            ),
            AnalysisRule(
                name="mark_outport_devices",
                func_name="_mark_outport_devices",
                # 需要通过网络反查器件 (net_device_map)
                requires=(
                    ("circuit.devices_dict", dict),
                    ("net_device_map", dict),
                ),
                description="[Tags] 标记输出端管、频率补偿电容、RC共模器件",
            ),
            AnalysisRule(
                name="mark_common_mode_detect_b",
                func_name="_mark_common_mode_detect_b",
                # 依赖 Phase 1 产生的 TAG_CM_OUTER 标签
                requires=(
                    ("tag_index", dict), 
                    ("circuit.devices_dict", dict),
                    ("net_device_map", dict),
                ...
                optional=True,
                description="[Tags/Group] 基于外侧管标签识别4管共模检测结构",
            ),
            AnalysisRule(
                name="analyze_diode_mos_structures",
                func_name="_analyze_diode_mos_structures",
                # 依赖 TAG_DIODE_MOS 标签
                requires=(
                    ("tag_index", dict),
                    ("circuit.devices_dict", dict),
                ...
                description="[Relations] 构建级联关系图(REL_CASCODE)与普通电流镜关系图(REL_STD)",
            ),
            AnalysisRule(
                name="mark_root_reference_path",
                func_name="_mark_root_reference_path",
                # 依赖 TAG_STD_MIRROR_REF 标签
                requires=(
                    ("tag_index", dict),
                    ("circuit.devices_dict", dict),
                    ("net_device_map", dict),
                ...
                optional=True,
                description="[Tags] 追踪并标记根参考管(TAG_ROOT_REF)",
            ),
            AnalysisRule(
                name="detect_low_voltage_current_mirrors",
                func_name="_detect_low_voltage_current_mirrors",
                # 依赖 TAG_STD_MIRROR_MIRROR (用于解耦) 和网络连接
                requires=(
                    ("circuit.devices_dict", dict),
                    ("net_device_map", dict),
                    ("tag_index", dict), # 隐式依赖
                ...
                optional=True,
                description="[Relations] 识别低压电流镜并更新关系图(REL_LV_...)",
            ),
            ...
            # ========================================================
            AnalysisRule(
                name="generate_current_paths",
                func_name="generate_current_paths",
                # 依赖 mark_obvious_tags 生成的 top_nodes
                requires=(
                    ("top_nodes", list),
                    ("net_device_map", dict),
                    ("circuit.devices_dict", dict),
                ),
                description="[Paths] 生成从电源到地的所有有效电流路径",
            ),
            AnalysisRule(
                name="analyze_current_beams",
                func_name="analyze_current_beams",
                # 依赖路径 + 关键角色标签 (DIFF_POS, OUTPORT_POS, CM_4MOS)
                requires=(
                    ("circuit.current_paths", list),
                    ("tag_index", dict),
                    ("net_device_map", dict), # 用于Beam 4 (Gate Net) 分析
                ),
                description="[Beams] 基于路径相似性和标签聚类生成电流束",
            ),
            AnalysisRule(
                name="mark_bias_mirrors",
                func_name="_mark_bias_mirrors",
                # 依赖电流束 (判断是否在主路) + 镜像关系图
                requires=(
                    ("circuit.current_beams", dict),
                    ("tag_index", dict),
                    ("relation_graph", dict),
                ),
                optional=True,
                description="[Tags] 标记偏置管(TAG_..._BIAS)",
            ),

            # ========================================================
            # Phase 3: 子结构登记
            # 依赖: 电流束数据 + 完整标签库 + 完整关系图
            # ========================================================
            
            # --- 3a. 关系驱动型 (Relation Driven) ---
            AnalysisRule(
                name="register_current_mirrors",
                func_name="_register_current_mirrors",
                # 纯粹依赖关系图
                requires=(
                    ("tag_index", dict),
                    ("relation_graph", dict),
                ...
                optional=True,
                description="[SubStruct] 登记普通电流镜",
            ),
            AnalysisRule(
                name="register_lv_mirrors",
                func_name="_register_lv_mirrors",
                requires=(
                    ("tag_index", dict),
                    ("relation_graph", dict),
                    ("circuit.devices_dict", dict),
                ...
                optional=True,
                description="[SubStruct] 登记低压电流镜",
            ),
            AnalysisRule(
                name="register_rc_structures",
                func_name="_register_rc_structures",
                requires=(("tag_index", dict),),
                optional=True,
                description="[SubStruct] 登记RC结构",
            ),

            # --- 3b. 电流束驱动型 (Beam Driven) ---
            AnalysisRule(
                name="register_beam_substructures",
                func_name="_register_beam_substructures",
                # 核心逻辑：结合电流束位置 + 标签 + 关系
                requires=(
                    ("circuit.current_beam_paths", dict), # 束路径
                    ("circuit.beam_net_sets", dict),      # 束网络
                    ("tag_index", dict),                  # 标签
                    ("relation_graph", dict),             # 关系 (用于Load A识别)
                    ("circuit.devices_dict", dict),
                ),
                optional=True,
                description="[SubStruct] 登记差分对/负载/跨束连接结构",
            ),
            AnalysisRule(
                name="register_lv_mirror_pairs",
                func_name="_register_lv_mirror_pairs",
                # 需要扫描电流束中的连续器件
                requires=(
                    ("circuit.current_beams", dict),
                    ("circuit.devices_dict", dict),
                ...
                optional=True,
                description="[SubStruct] 登记低压镜像对",
            ),
            AnalysisRule(
                name="register_sym_capacitors",
                func_name="_register_sym_capacitors",
                # 依赖电流束路径判断对称性
                requires=(
                    ("circuit.devices_dict", dict),
                    ("circuit.current_beam_paths", dict),
                    ("net_device_map", dict),
                ),
                optional=True,
                description="[SubStruct] 登记对称电容",
            ),

            # --- 3c. 衍生分组型 (Group Derived) ---
            AnalysisRule(
                name="get_tail_current",
                func_name="_get_tail_current",
                # 依赖 Beam 2/3 的路径 + 之前步骤产生的标签(TAG_LOAD_TYP, TAG_LV_PAIR)
                requires=(
                    ("circuit.current_beam_paths", dict),
                    ("circuit.devices_dict", dict),
                    ("tag_index", dict), # 隐式依赖
                ...
                optional=True,
                description="[Groups] 识别尾电流源",
            ),
            AnalysisRule(
                name="get_output_pair",
                func_name="_get_output_pair",
                # 依赖 Beam 2 的网络集合 + STR_OUTPORT_PAIR 分组
                requires=(
                    ("circuit.beam_net_sets", dict),
                    ("device_groups", dict),
                    ("circuit.devices_dict", dict),
                ),
                optional=True,
                description="[SubStruct] 聚类逻辑输出对",
            ),
            ...
            # ========================================================
            AnalysisRule(
                name="generate_constraint_groups",
                func_name="_generate_constraint_groups",
                # 依赖所有已登记的子结构
                requires=(
                    ("circuit.substructures", list),
                    ("circuit.devices_dict", dict),
                ),
                optional=True,
                description="[Constraint] 生成参数关联组",
            ),
            AnalysisRule(
                name="maybe_calibrate",
                func_name="_maybe_calibrate",
                # 依赖参数库 + 约束组 + 标签(优先级) + 关系图(复制管)
                requires=(
                    ("device_params", dict),
                ...
                optional=True,
                description="[Calibration] 执行两阶段参数校准",
            ),
            AnalysisRule(
                name="normalize_outputs",
                func_name="_normalize_outputs_deterministically",
                requires=(),
                optional=True,
                description="[Output] 产物归一化",
            ),
        ]
    ...
    def _run_rules(self, rules: List[AnalysisRule]) -> None:
        ...
        for rule in rules:
            ok, missing = self._check_rule_requires(rule.requires)
            if not ok:
                msg = "规则前置条件不满足"
                # [P1] requires 可能包含 (path, type)；为避免 report/log 序列化与类型检查问题，统一格式化
                formatted_requires: List[Any] = []
                for r in rule.requires:
                    if isinstance(r, tuple) and len(r) == 2:
                        formatted_requires.append({"path": r[0], "type": getattr(r[1], "__name__", str(r[1]))})
                    else:
                        formatted_requires.append(r)
                ctx: Dict[str, Any] = {"rule": rule.name, "missing": missing, "requires": formatted_requires}
                if rule.optional:
                    self._warn(msg, **ctx)
                    ctx_rec = dict(ctx)
                    ctx_rec["reason"] = "missing_requires"
                    skipped.append(ctx_rec)
                    continue
                else:
                    self._error(msg, **ctx)
                    ctx_rec = dict(ctx)
                    ctx_rec["reason"] = "missing_requires"
                    failed.append(ctx_rec)
                    continue

            try:
                fn = getattr(self, rule.func_name)
            except AttributeError:
                msg = "规则函数不存在"
                ctx: Dict[str, Any] = {"rule": rule.name, "func_name": rule.func_name}
                if rule.optional:
                    self._warn(msg, **ctx)
                    ctx_rec = dict(ctx)
                    ctx_rec["reason"] = "missing_func"
                    skipped.append(ctx_rec)
                    continue
                else:
                    self._error(msg, **ctx)
            ...
            self._info("执行规则", rule=rule.name, func_name=rule.func_name)
            try:
                fn()
                executed.append(rule.name)

                # [Audit] 可选的规则级状态快照与一致性校验（默认关闭）
                self._record_state_snapshot(f"after_rule:{rule.name}")
                self._validate_state(f"after_rule:{rule.name}")
            except Exception as e:
                tb = traceback.format_exc()
                self._error("规则执行异常", exc_info=True, rule=rule.name, func_name=rule.func_name, error=str(e), traceback=tb)
                failed.append({"rule": rule.name, "func_name": rule.func_name, "reason": "exception", "error": str(e)})
    ...
    def _check_rule_requires(self, requires: Tuple[Union[str, Tuple[str, type]], ...]) -> Tuple[bool, List[str]]:
        """检查属性路径是否存在、可达，且符合预期类型（如果指定）。"""
        missing: List[str] = []
        for item in requires:
            # 解析配置
            if isinstance(item, tuple):
                path, expected_type = item
            else:
                path, expected_type = item, None

            if not path: continue

            # 1. 检查存在性
            val = self._get_attr_path_value(path) # 需要新增一个获取值的辅助函数
            if val is None: # 这里假设 None 视为缺失，视具体业务逻辑而定
                missing.append(f"{path} (Missing)")
                continue
            
            # 2. 检查类型 (新增安全特性)
            if expected_type is not None and not isinstance(val, expected_type):
                missing.append(f"{path} (TypeMismatch: expected {expected_type.__name__}, got {type(val).__name__})")
        
        return (len(missing) == 0), missing
    
    def _get_attr_path_value(self, path: str) -> Any:
        """辅助：递归获取路径的值，失败返回 None"""
        obj: Any = self
        try:
    ...
    def analyze_current_beams(self):
        ...
        **依赖4管共模检测标签、差分输入标签、输出端管标签、电流路径生成**
        ...
        # [Audit] 电流束更新后必须失效束缓存
        self._invalidate_beam_cache("analyze_current_beams")
    ...
    def _calibrate_device_params(self):
        ...
        for group in groups_to_process:
            ...
            if golden_ref_param:
                ...
                if not values_float:
                    self._warn("约束组无可解析值，跳过", group=group)
                    continue
            ...
            if golden_val_str is None:
                self._warn("无法确定组黄金标准值", group=group, key=group[0])
                continue
            ...
            for (fw_now, m_now, l_now_unused), dev_group in param_groups.items():
                ...
                try:
                    ...
                    if ref_fw_f is None or ref_m_f is None:
                        self._warn("参考管几何参数解析失败，无法补偿", ref=ref_dev_name, fw=ref_fw_str, m=ref_m_str)
                        continue
                    ...
                    if W_ref_now_float == 0.0:
                        self._warn("参考管当前 W=0，无法补偿", ref=ref_dev_name)
                        continue
                    ...
                    tb = traceback.format_exc()
                    self._error("复制管补偿组失败", exc_info=True, group=dev_group, error=str(e), traceback=tb)
...
if __name__ == "__main__":
    ...
    # 如果文件在同目录下，直接写文件名即可
    netlist_path = "case2_netlist_generated.txt" 
    # =========================================
    ...
    if not os.path.exists(netlist_path):
        ...
        try:
            ...
            devices_to_check.extend(["PM4","PM5","PM7","PM9"])
