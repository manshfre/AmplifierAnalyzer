...
class CalibrationConfig:
    """该类用于参数校准部分的一些阈值或标志设置，与具体的参数校准逻辑有关"""
    # 格式化规则
...
class AnalysisStage:
    """分析阶段格式设置，是分析器流水线的一级"""
    name: str   #该级流水线名称
    func_name: str  #该级流水线工作函数
    requires: Tuple[str, ...] = ()  # 该级流水线加工的数据（如 'circuit.current_paths'）
    optional: bool = False          # 该级流水线是否可以撤掉：True 时前置条件不满足则跳过；False 则记录为错误并继续
    description: str = ""   #该级流水线详细描述
...
class CircuitAnalyzer:
    ...
    def from_spice_netlist(
        ...
        """
        分析器与网表的对接接口，接收网表，使用特定解析器解析出:
        1.格式固定的器件信息
        2.参数配置完整性信息
        
        Args:
            netlist_path: SPICE网表文件路径
            parser_class: 自定义解析器类（None使用DefaultSpiceParser）
        
        Returns:
            CircuitAnalyzer实例
        """
        # 在解析前，先实例化配置对象，APPConfig配置应与目标网表对齐，因为解析器会用到
    ...
    def from_parsed_data(cls, devices_information: List[List[Any]], config_complete: bool, app_config: Optional[APPConfig] = None):
        """
        分析器校验接口，校验网表解析数据，实例化Device，创建分析器
        ...
        for idx, dev_info in enumerate(devices_information):
            ...
            # 构建网络-器件映射
            nets = set(terminals_lower.values())  # 避免字典中有重复net键
            for net in nets:
    ...
    def _run_analysis_pipeline(self):
        """
        分析器工作接口，设置多级流水线，依次对数据进行处理

        - 使用AnalysisStage类，每级流水线具备名称、工作函数名称、前置条件、可选性与描述
        """
        # 允许实例重复运行：每次运行前清理派生态与报告
        self._prepare_for_pipeline_run()

        # 构建流水线
        rules = self._build_default_stages()
        
        # 运行流水线
        self._run_pipeline(rules)
    ...
    def _reset_report_for_run(self) -> None:
        """为重复 pipeline run 重置 report（保证可重入且不累积污染）
    ...
    def _record_state_snapshot(self, stage: str) -> None:
        ...
        snapshot = {
            "stage": stage, #名称
            "devices": len(self.circuit.devices_dict),  #器件数量
            "tags_idx_count": len(self.tag_index),  #标签识别数量
            "groups_count": len(self.device_groups),    #组合识别数量、不含层级关系的子结构类型识别数量
            "relations_src_count": len(self.relation_graph),    #含层级关系的子结构类型识别数量
            "substructures": len(self.circuit.substructures),   #识别的子结构实例数量
            "paths": len(self.circuit.current_paths),   #电流束路径数量
            "beams": len(self.circuit.current_beams)    #电流束数量，应与电流束路径数量相等
        }
    ...
    # --------------------------------3.2流水线构建与运行---------------------------------
    # -------------------------------------------------------------------------------
    def _build_default_stages(self) -> List[AnalysisStage]:
        """
        构建分析器流水线工作阶段。
        ...
        def r(name, func, reqs, desc, opt=False):
            ...
            return AnalysisStage(name, func, tuple(reqs), opt, desc)
    ...
    def _run_pipeline(self, rules: List[AnalysisStage]) -> None:
        """
        运行流水线，并将执行情况写入 report.stats
        executed代表正常工作的工作函数数量
        skipped代表非必需且发生异常的工作函数数量
        failed代表必需且发生异常的工作函数数量
        """
        executed: List[str] = []
    ...
    def _normalize_outputs_deterministically(self) -> None:
        ...
        # 5) Circuit 的电流相关产物：仅排序整体字典的索引顺序
    ...
    def get_groups(self, group_key: str) -> List[List[str]]:
        """获取特定组合器件名"""
        return self.device_groups.get(group_key, []).copy()
    ...
    def export_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            json_str = json.dumps(raw, default=self._json_default, sort_keys=True, ensure_ascii=False)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(netlist_path):
        ...
        try:
            ...
            report1 = analyzer.export_report()
            print(report1.get("digest",""))

            analyzer._run_analysis_pipeline()
            report2 = analyzer.export_report()
            print(report2.get("digest",""))
